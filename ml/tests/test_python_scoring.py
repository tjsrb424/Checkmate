...
def test_initial_board_scores_with_han_deom():
    ...
    assert score_board_material(position.board) == {"cho": 72.0, "han": 73.5, "winner": "HAN"}
...
def test_removed_chariot_changes_winner():
    ...
    assert score_board_material(position.board) == {"cho": 72.0, "han": 60.5, "winner": "CHO"}
...
def test_han_deom_applies_on_bare_generals():
    ...
    assert score_board_material(board) == {"cho": 0.0, "han": 1.5, "winner": "HAN"}
