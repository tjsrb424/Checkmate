...
import { exportPolicySamplesFromOpeningRecords, parseOpeningRecordsCsv, policySamplesToJsonl } from '../src/engine';

interface ExportPolicyCliOptions {
  input: string;
  output: string;
  summary: string;
  maxPly: number;
...
const defaultOptions: ExportPolicyCliOptions = {
  input: 'data/processed/janggi_clean_records.csv',
  output: 'data/ml/policy_samples.jsonl',
  summary: 'data/ml/policy_samples_summary.json',
  maxPly: 16,
...
function main(): void {
  try {
    ...
    const exported = exportPolicySamplesFromOpeningRecords(records, {
    ...
    writeFileSync(outputPath, policySamplesToJsonl(exported.samples), 'utf8');
    writeFileSync(
      summaryPath,
      JSON.stringify(
        {
    ...
    console.log('Policy data export complete');
...
function parseArgs(args: string[]): ExportPolicyCliOptions {
  const options: ExportPolicyCliOptions = { ...defaultOptions };
