...
monitor_progress() {
  ...
  while kill -0 "$pid" >/dev/null 2>&1; do
    ...
    write_progress "autotrain_running" "$estimated" "AutoTrain A2 running. Percent is elapsed-time based, not exact internal game/batch progress."
    sleep 30
  done
}
...
main() {
  ...
  send_webhook "Oetongsu RunPod A2 started: preflight"
  ...
  write_progress "autotrain_start" 15 "Starting AutoTrain A2"
  run_autotrain_a2
  ...
  if [ "$autotrain_status" -eq 0 ]; then
    ...
    write_progress "completed" 100 "A2 complete. Artifact packaged. Auto Stop requested."
    send_webhook "Oetongsu RunPod A2 completed: artifact packaged. Stop requested."
  else
    JOB_STATUS="failed"
    write_progress "failed" 100 "A2 failed or timed out. Partial artifact packaged. Stop requested."
    send_webhook "Oetongsu RunPod A2 failed or timed out: partial artifact packaged. Stop requested."
  fi
